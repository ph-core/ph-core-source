...
//이름 처리
$match = REGEXP_NICK;
$kakao_inf['name'] = '픽플회원'.rand(1,999);
if($kakao_arr['name']){
...
//가입여부 확인
$sql->query(
    $sql->scheme->member('select:mb_sns_ka_id_inspt'),
    array(
        $kakao_arr['id']
    )
);
...
if($sql->getcount()>0){
    $sql->query(
        $sql->scheme->member('update:sns_login_update_token_ka'),
        array(
...
//로그인 정보 로드
$sql->query(
    $sql->scheme->member('select:mb_sns_ka_signin'),
    array(
        $kakao_arr['id']
    )
);
