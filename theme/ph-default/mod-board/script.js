...
function view_cmt_load(){
  ...
  $('#board-comment').load(MOD_BOARD_DIR+'/controller/comment?OUTLOAD=1&board_id='+comment_board_id+'&read='+comment_read);
}
$(function(){
  view_cmt_load();
});
  ...
  $(document).on('click','#cmt-reply',function(e){
    ...
    var vis = $('> #comm-re-form',$(this).parents('li.comm-list-li')).is(':visible');
      ...
      $('#commentForm input[name=cidx]').val($(this).data("cmt-reply"));
  		$(this).parents('li.comm-list-li').append($comm_re_form);
